...
class StatsAggregationUtils:
    """Shared utilities for statistics aggregation."""
    
    @staticmethod
    def initialize_daily_data(start_date, end_date):
        """Initialize daily aggregation containers optimized for performance."""
        ...
        # Optimized date range creation
        date_range = [(start_date + timedelta(days=i)).isoformat() 
        ...
        return daily_tokens, daily_costs, daily_cost_breakdown
    
    @staticmethod
    def aggregate_date_range(stats, earliest_timestamp, latest_timestamp):
    ...
    @staticmethod
    def aggregate_daily_data(stats, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date):
...
class GlobalStatsAggregator:
    ...
    async def get_global_stats(self, projects: list[dict]) -> dict:
        ...
        # Initialize daily data with zeros using shared utility
        daily_tokens, daily_costs, daily_cost_breakdown = StatsAggregationUtils.initialize_daily_data(start_date, end_date)
        ...
        for project in projects:
            ...
            if stats:
                ...
                # Aggregate daily stats using shared utility
                StatsAggregationUtils.aggregate_daily_data(stats, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date)

                # Track earliest and latest usage using shared utility
                earliest_timestamp, latest_timestamp = StatsAggregationUtils.aggregate_date_range(stats, earliest_timestamp, latest_timestamp)
            else:
    ...
    def _initialize_daily_data(self, start_date, end_date):
        """Initialize daily aggregation containers."""
        return StatsAggregationUtils.initialize_daily_data(start_date, end_date)

    def _initialize_aggregation_containers(self):
        """Initialize all aggregation containers."""
        return {
            # Overview stats
            "total_input": 0,
            "total_output": 0,
            ...
            # Cache
            "cache_stats": {"created": 0, "read": 0, "messages_with_read": 0, 
                           "messages_with_created": 0, "assistant_messages": 0},
            
            # Sessions
            "sessions_stats": {"durations": [], "error_counts": [], "message_counts": []}
        }

    def _aggregate_overview_stats(self, stats, containers):
        """Aggregate overview statistics from a project."""
        overview = stats.get("overview", {})
        total_tokens = overview.get("total_tokens", {})
        
        containers["total_input"] += total_tokens.get("input", 0)
        containers["total_output"] += total_tokens.get("output", 0)
        containers["total_cache_read"] += total_tokens.get("cache_read", 0)
        containers["total_cache_write"] += total_tokens.get("cache_creation", 0)
        containers["total_messages"] += overview.get("total_messages", 0)
        containers["total_sessions"] += overview.get("sessions", 0)
        containers["total_cost"] += overview.get("total_cost", 0)
        
        # Aggregate message types
        msg_types = overview.get("message_types", {})
        for msg_type, count in msg_types.items():
            if msg_type in containers["message_types"]:
                containers["message_types"][msg_type] += count

    def _aggregate_user_interactions(self, stats, containers):
        """Aggregate user interaction statistics."""
        user_interactions = stats.get("user_interactions", {})
        containers["total_commands"] += user_interactions.get("user_commands_analyzed", 0)
        
        # Calculate weighted average for steps
        command_count = user_interactions.get("user_commands_analyzed", 0)
        avg_steps = user_interactions.get("avg_steps_per_command", 0)
        containers["total_steps"] += avg_steps * command_count
        
        # Aggregate distributions
        tool_dist = user_interactions.get("tool_count_distribution", {})
        for count, freq in tool_dist.items():
            containers["tool_count_distributions"][count] = containers["tool_count_distributions"].get(count, 0) + freq
        
        model_dist = user_interactions.get("model_distribution", {})
        for model, freq in model_dist.items():
            containers["model_distributions"][model] = containers["model_distributions"].get(model, 0) + freq
        
        # Aggregate tool and interruption stats
        containers["interruption_stats"]["commands"] += command_count
        containers["interruption_stats"]["interruptions"] += user_interactions.get("commands_followed_by_interruption", 0)
    ...
    def _aggregate_tools_stats(self, stats, containers):
        ...
        for tool, count in usage_counts.items():
            containers["tools_usage"][tool] = containers["tools_usage"].get(tool, 0) + count
        
        # Aggregate error counts
        error_counts = tools_section.get("error_counts", {})
        for tool, count in error_counts.items():
            containers["tools_errors"][tool] = containers["tools_errors"].get(tool, 0) + count

    def _aggregate_errors_stats(self, stats, containers):
        """Aggregate error statistics."""
        errors_section = stats.get("errors", {})
        containers["total_errors"] += errors_section.get("total", 0)
        
        error_cats = errors_section.get("by_category", {})
        for category, count in error_cats.items():
            containers["error_categories"][category] = containers["error_categories"].get(category, 0) + count

    def _aggregate_models_stats(self, stats, containers):
        """Aggregate model usage statistics."""
        models_section = stats.get("models", {})
        for model, model_stats in models_section.items():
            if model not in containers["models_stats"]:
                containers["models_stats"][model] = {
                    "count": 0, "input_tokens": 0, "output_tokens": 0,
                    "cache_creation_tokens": 0, "cache_read_tokens": 0, "cost": 0
                }
            model_container = containers["models_stats"][model]
            model_container["count"] += model_stats.get("count", 0)
            model_container["input_tokens"] += model_stats.get("input_tokens", 0)
            model_container["output_tokens"] += model_stats.get("output_tokens", 0)
            model_container["cache_creation_tokens"] += model_stats.get("cache_creation_tokens", 0)
            model_container["cache_read_tokens"] += model_stats.get("cache_read_tokens", 0)
            model_container["cost"] += model_stats.get("cost", 0)

    def _aggregate_cache_stats(self, stats, containers):
        """Aggregate cache statistics."""
        cache_section = stats.get("cache", {})
        cache_stats = containers["cache_stats"]
    ...
    def _aggregate_sessions_stats(self, stats, containers):
        """Aggregate session statistics."""
        sessions_section = stats.get("sessions", {})
        sessions_stats = containers["sessions_stats"]
    ...
    def _aggregate_daily_stats(self, stats, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date):
        """Aggregate daily statistics."""
        StatsAggregationUtils.aggregate_daily_data(stats, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date)

    def _update_date_range(self, stats, containers):
        """Update the earliest and latest timestamps."""
        containers["earliest_timestamp"], containers["latest_timestamp"] = StatsAggregationUtils.aggregate_date_range(
            stats, containers["earliest_timestamp"], containers["latest_timestamp"]
        )
    ...
    async def get_rollup_stats(self, rollup_name: str, child_projects: list[dict]) -> dict:
        ...
        # Initialize date range for daily aggregation
        from datetime import datetime, timedelta
        end_date = datetime.now().date()
        start_date = end_date - timedelta(days=29)
        
        # Initialize all containers
        containers = self._initialize_aggregation_containers()
        daily_tokens, daily_costs, daily_cost_breakdown = self._initialize_daily_data(start_date, end_date)
        ...
        for project in child_projects:
            ...
            if stats:
                # Use focused helper methods for aggregation
                self._aggregate_overview_stats(stats, containers)
                self._aggregate_user_interactions(stats, containers)
                self._aggregate_tools_stats(stats, containers)
                self._aggregate_errors_stats(stats, containers)
                self._aggregate_models_stats(stats, containers)
                self._aggregate_cache_stats(stats, containers)
                self._aggregate_sessions_stats(stats, containers)
                self._aggregate_daily_stats(stats, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date)
                self._update_date_range(stats, containers)
        
        # Build final rollup statistics using helper method
        return self._build_rollup_response(
            rollup_name, containers, daily_tokens, daily_costs, daily_cost_breakdown,
            start_date, end_date, len(child_projects)
        )

    def _convert_daily_data_to_lists(self, daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date):
        """Convert daily data dictionaries to lists for response format."""
        from datetime import timedelta
        
        daily_token_list = []
        daily_cost_list = []
        
        # Optimized date range iteration
        date_range = [(start_date + timedelta(days=i)).isoformat() 
                      for i in range((end_date - start_date).days + 1)]
        ...
        return daily_token_list, daily_cost_list

    def _calculate_derived_metrics(self, containers):
        ...
        tools_error_rates = {}
        for tool in containers["tools_usage"]:
            error_count = containers["tools_errors"].get(tool, 0)
            usage_count = containers["tools_usage"][tool]
        ...
        error_rate = (containers["total_errors"] / containers["total_messages"]) if containers["total_messages"] > 0 else 0
    ...
    def _build_rollup_response(self, rollup_name, containers, daily_tokens, daily_costs, 
                              daily_cost_breakdown, start_date, end_date, project_count):
        """Build the final rollup statistics response."""
        from datetime import timedelta
        
        # Convert daily data to lists
        daily_token_list, daily_cost_list = self._convert_daily_data_to_lists(
            daily_tokens, daily_costs, daily_cost_breakdown, start_date, end_date
        )
        
        # Calculate derived metrics
        metrics = self._calculate_derived_metrics(containers)
        
        # Extract values from containers for cleaner code
        cache_stats = containers["cache_stats"]
        interruption_stats = containers["interruption_stats"]
        ...
        rollup_stats = {
            "overview": {
                ...
                "total_tokens": {
                    "input": containers["total_input"],
                    "output": containers["total_output"],
                    "cache_read": containers["total_cache_read"],
                    "cache_creation": containers["total_cache_write"]
                },
                "total_cost": containers["total_cost"],
                "date_range": {
                    "start": containers["earliest_timestamp"].isoformat() if containers["earliest_timestamp"] else None,
                    "end": containers["latest_timestamp"].isoformat() if containers["latest_timestamp"] else None
                },
                "message_types": containers["message_types"],
                "total_messages": containers["total_messages"],
                "sessions": containers["total_sessions"]
            },
            "user_interactions": {
                ...
                "percentage_requiring_tools": round((containers["commands_with_tools"] / containers["total_commands"] * 100), 1) if containers["total_commands"] > 0 else 0,
            ...
            "tools": {
                ...
                "error_counts": containers["tools_errors"],
                "error_rates": metrics["tools_error_rates"]
            },
            "errors": {
                "total": containers["total_errors"],
                "rate": round(metrics["error_rate"], 1),
                "by_category": containers["error_categories"],
                "assistant_details": []  # No individual messages for rollups
            },
            "models": containers["models_stats"],
            "cache": {
                ...
                "hit_rate": round(metrics["cache_hit_rate"], 1),
                "efficiency": round(min(100, metrics["cache_efficiency"]), 1),
                "tokens_saved": metrics["tokens_saved"],
                "cost_saved_base_units": round(metrics["cache_cost_saved"], 2),
            ...
            "sessions": {
                "count": containers["total_sessions"],
                "average_duration_seconds": metrics["avg_session_duration"],
                "average_messages": metrics["avg_session_messages"],
                "sessions_with_errors": metrics["total_error_sessions"]
            },
            "hourly_pattern": {
            ...
            "last_message_date": containers["latest_timestamp"].isoformat() if containers["latest_timestamp"] else None,
            "is_rollup": True,
            "rollup_name": rollup_name,
            "child_project_count": project_count
        }
        
        logger.info(
            f"Rollup '{rollup_name}' aggregation complete: {project_count} projects, "
            f"{containers['total_commands']} commands, {containers['total_input'] + containers['total_output']} total tokens, "
            f"${containers['total_cost']:.2f} total cost"
        )
